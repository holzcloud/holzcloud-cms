...
- [x] 10-06-PLAN.md — the second factor: one predicate, five call sites, and the dependency shown on two admin screens (SSO-07)
- [x] 10-07-PLAN.md — the sign-out, and the CSP pair this shape does not need (SSO-08)

**Wave 7** *(blocked on Wave 6)*

- [x] 10-08-PLAN.md — the deployment documents: the Caddyfile with one explicit delete per copied header, the minimum Caddy version, and the operator's two verifications (SSO-07, SSO-11)

**Wave 8** *(blocked on Wave 7)*

- [x] 10-09-PLAN.md — the catalogues, closed before the browser pass opens (QUAL-01)

**Wave 9** *(blocked on Wave 8, and on the code-review fix round)*

- [x] 10-10-PLAN.md — three browser passes: forward auth on, forward auth off, and the v1.6 milestone close-out (SSO-01, SSO-02, SSO-08, SSO-09, QUAL-01, QUAL-02)
...
- **Other pitfalls that must land in the plan:** logout that does not log out (the browser still holds the authentik cookie and is signed straight back in — redirect to the outpost's `sign_out`. **This sentence used to say "build the target from the request's own host, never from a header", and it was wrong twice** — `r.Host` *is* a header value, so the instruction contradicts itself; and plan 10-07 implemented it literally as a mutation and got five red assertions, one of them `form-action 'self' will block it silently in Safari`. The target is a **relative path from the configuration**, validated once at load to begin with exactly one slash, and the handler reads nothing from the request to build it. **Nor does `auth.SafeReturn` fit**, which this line also used to recommend: measured, `SafeReturn("/outpost.goauthentik.io/sign_out")` returns `"/admin/"`, so following it would have shipped a sign-out button that lands the person back inside the administration, with a green build; and use `HX-Redirect` + `Vary: HX-Request`, because logout is an htmx POST). **CSRF stays on every route** — forward auth makes CSRF *more* relevant, not less, because it adds a second ambient credential the browser attaches automatically. Group → role mapping must split on `|` and compare **whole elements** (a naive `strings.Contains` matches `not-holzcloud-admins`), must run on every request so demotion works, must log every role change, and the admin-group variable must have **no default**. E-mail is not a stable identity: SQLite's `COLLATE NOCASE` folds ASCII only, so `Müller@` and `müller@` are two rows and two admins. And the password path must survive, because `auth.RequireFreshPassword` (`elevate.go:57`) guards website deletion, user deletion, AI key creation and plugin removal by asking for a password an SSO account does not have.
