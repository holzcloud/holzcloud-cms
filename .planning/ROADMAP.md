...
- **Ordering constraint, non-negotiable:** rebuild-and-hash-compare in CI **first**, promote the skips **second**. Promoting first gives a contributor with a partial checkout a red suite they cannot fix and closes nothing, because the hole is that CI never rebuilds. Shape: `plugins/build.sh` + a CI step + `HOLZCLOUD_TEST_REQUIRE_WASM=1`.
- **CI budget matters.** `security.yml:38–41` records `go test -race ./internal/plugin/` **alone at 297 s**. Adding five wasm builds to CI has a cost; budget it in the plan rather than discovering it in a queue.
