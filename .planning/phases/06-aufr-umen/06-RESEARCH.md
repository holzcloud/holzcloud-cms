...
| Acquiring a specific Go version | Downloading a tarball in CI, or a second `setup-go` step | `GOTOOLCHAIN=go1.26.6` | The go command downloads it as a module, proxied through `GOPROXY` and checksum-verified [CITED: go.dev/doc/toolchain]. This machine is already running such a toolchain: `GOROOT=/Users/dev/go/pkg/mod/golang.org/toolchain@v0.0.1-go1.26.6.darwin-arm64` [VERIFIED: `go env GOROOT`, this session]. |
