...
- [ ] **MAINT-03**: CI rebuilds every `plugins/*/plugin.wasm` and compares the result against the committed file, so a change to the SDK can no longer be validated against a stale binary.
...
- [ ] **MAINT-05**: The planning and repository notes that have gone stale are corrected: `docs/offene-punkte.md` says migrations run to `00044` (they run to `00045`) and still lists the finished Dependabot item as work; all three `deferred-items.md` read as open although all three are closed; `.planning/codebase/ARCHITECTURE.md` carries six drifted facts.
