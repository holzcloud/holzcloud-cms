...
### ✅ v2.5 — On the Phone (opened and closed 2026-09-17)
...
- [x] **19-04** — PHONE-04 and PHONE-05: measured again and driven by hand.
      1048 → 0 pixels past the edge, 86 → 0 unheld boxes, 462 → 0 controls
      under the minimum, 16 → 0 fields a phone zooms for. The hands-on pass
      found two, both real: a `position: static` for the row menu that had
      never run because it sat one cascade layer too early, so the last row's
      menu was cut off below its card; and a `display: grid` this phase had
      added an hour before, which put a drawer handle beside an open drawer on
      a touch laptop. See `19-04-VERIFICATION.md`.

---

## Progress

Phases 13 to 19 are complete; v2.1 to v2.5 are closed. Numbering continues from 20; it never restarts.

Phase 19 is the one to remember for a different reason than phase 18. Phase 18
found eleven defects by writing tests, and its browser pass found nothing — the
first time that had happened. Phase 19 is the counterweight: its browser pass
found the two things that mattered most, and both were rules that **looked**
right in the file. One had never run in its life because it sat one cascade
layer too early; the other was written an hour before, by this phase, and put a
drawer handle beside an open drawer. Neither would ever fail a test, because
nothing fails — the page just looks wrong to somebody holding it.

Phase 18 stays worth remembering too: eleven defects, every one found by writing
