...
| `HOLZCLOUD_LISTEN` | `127.0.0.1` | The address the server binds. Loopback by default, which suits a proxy on the same host; a proxy elsewhere needs `0.0.0.0` (or `::`). The container image sets `0.0.0.0` itself |
